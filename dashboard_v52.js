...
function currentGregorianIso() {
  return ymd(datePartsInZone(new Date()));
}

function renderGregorianDateNow() {
  const el = $("gregorian-date");
  if (el) el.textContent = humanDate(currentGregorianIso(), {weekday:true});
}

function findNextMidnightInConfiguredZone() {
  const now = Date.now();
  const today = ymd(datePartsInZone(new Date(now)));

  // Find a point after the next local date boundary, then binary-search back
  // to approximately the first second of the new civil day. This is based
  // entirely on CONFIG.timezone, not the computer's own timezone.
  let lo = now;
  let hi = now + 27 * 3600000;
  while (ymd(datePartsInZone(new Date(hi))) === today) hi += 6 * 3600000;

  while (hi - lo > 1000) {
    const mid = Math.floor((lo + hi) / 2);
    if (ymd(datePartsInZone(new Date(mid))) === today) lo = mid;
    else hi = mid;
  }
  return new Date(hi);
}

function scheduleGregorianMidnightBoundary() {
  if (gregorianMidnightTimer) clearTimeout(gregorianMidnightTimer);
  gregorianMidnightTimer = null;

  renderGregorianDateNow();
  const target = findNextMidnightInConfiguredZone();
  const delay = Math.max(1000, target.getTime() - Date.now() + 1000);

  gregorianMidnightTimer = setTimeout(() => {
    // Advance the visible Gregorian date immediately at midnight in the
    // selected location, then refresh all date-dependent dashboard data.
    renderGregorianDateNow();
    refreshDashboard();
  }, delay);
}
...
function hoshanotForSukkotDay(hDay, wday) {
  // Ashkenaz Hoshanot sequence.  The order varies with the weekday on
  // which 15 Tishrei falls.  Shabbat and Hoshana Rabbah use the
  // dashboard's requested descriptive labels.
  if (hDay < 15 || hDay > 21) return "";
  if (hDay === 21) return "Hoshanot for Hoshana Raba";
  if (wday === 7) return "Hoshanot for Shabbat";

  const offset = hDay - 15;
  const firstDayWday = ((wday - 1 - offset) % 7 + 7) % 7 + 1;
  const sequences = {
    2: [1, 2, 3, 6, 4, 7],       // Sukkot begins Monday
    3: [1, 2, 3, 4, 7, 5],       // Tuesday
    5: [1, 2, 7, 3, 4, 5],       // Thursday
    7: [7, 1, 3, 2, 4, 5],       // Shabbat
  };
  const names = {
    1: "Lemaan Amitach",
    2: "Even Shetiyah",
    ...
    5: "Adon HaMoshiah",
    6: "Om Ani Chomah",
    7: "Hoshanot for Shabbat",
  };
  const seq = sequences[firstDayWday];
  if (!seq || offset < 0 || offset >= seq.length) return "";
  const value = names[seq[offset]] || "";
  return value.startsWith("Hoshanot") ? value : `Hoshanot: ${value}`;
}
...
function buildUrls(today, lookahead) {
  ...
  return {
    ...
    calendar: `${HEB}/hebcal?v=1&cfg=json&maj=on&min=on&mod=on&nx=on&ss=on&s=on&mf=on&c=on&F=on&dpa=on&d=on&b=on&molad=on&mvch=on&i=${israel}&${loc}&start=${today}&end=${lookahead}`,
  };
}
...
async function calculateDashboard() {
  ...
  if (wday === 6) {
    ...
    // Ashkenazic practice omits Vi'hi Noam when an actual Yom Tov occurs on
    // a weekday (Sunday-Friday) during the coming week.
    if (!yomTovTomorrow) {
      ...
      extraMaar.push("Festival Amidah", "Va\'Todiainu");
    }
  ...
  const hoshanotMusaf = /^Tishrei$/i.test(hMonth)
    ? hoshanotForSukkotDay(hDay, wday)
    : "";
  ...
  if (wday === 7 && minchaHasFestivalAmidah) {
    ...
    minchaElements.push(...extraMincha.filter(x => /^Torah:/i.test(x)));
    minchaElements.push(...extraMincha.filter(x => !/^Torah:/i.test(x)));
  } else {
...
function renderDashboard(data) {
  ...
  // Gregorian date always follows the selected location's timezone.
  renderGregorianDateNow();
...
function selectLocation(result) {
  ...
  if (gregorianMidnightTimer) clearTimeout(gregorianMidnightTimer);
  lastData = null;
  // Change the visible clock/date immediately using the NEW location's timezone,
  // before waiting for any network response.
  updateClock();
  scheduleGregorianMidnightBoundary();
  refreshDashboard();
}
